...
// const zonaUsuario = dayjsSpanish.tz.guess();

const transformValidDate = (date?: ConfigType) => {
  const isValid = date && dayjsSpanish(date).isValid();
  if (!isValid) return dayjsSpanish();

  // return dayjsSpanish.utc(date).tz(zonaUsuario);
  return dayjsSpanish.utc(date).local();
};
...
export const formatDateToUtcMidnight = (date?: ConfigType) => {
  ...
  return dayjsSpanish.utc(date).startOf("day").toISOString();
};
